...
def identity_block(X_input, kernel_size, in_filter, out_filters, stage, block):
    ...
    """
    #(X_input, kernel_size, in_filter, out_filters, stage, block)
    # defining name basis

    block_name = 'res' + str(stage) + block
    f1, f2, f3 = out_filters # out_filters=[64, 64, 256]
    with tf.variable_scope(block_name):
        X_shortcut = X_input

        # first
        W_conv1 = weight_variable([1, 1, in_filter, f1]) # in_filter = 256
...
def convolutional_block(X_input, kernel_size, in_filter,
    ...
    with tf.variable_scope(block_name):
        ...
        x_shortcut = tf.nn.conv2d(X_input, W_shortcut, strides=[1, stride, stride, 1], padding='VALID')

        # final
        add = tf.add(x_shortcut, X)
        # 建立最后融合的权重
        # b_conv_fin = bias_variable([f3]) 都没有W , 加b反而造成相当大的误差
        add_result = tf.nn.relu(add)
...
# stage 2
x2 = convolutional_block(X_input=x1, kernel_size=3, in_filter=64, out_filters=[64, 64, 256], stage=2, block='a',
                         stride=1)
...
for i in range(1000):
    ...
    batch_2 = mnist.test.next_batch(10)
    acc_+=accuracy.eval(feed_dict={x:batch_2[0], y:batch_2[1], keep_prob: 1.0})
    # print("test accuracy %g" % accuracy.eval(feed_dict={x:batch_2[0], y:batch_2[1], keep_prob: 1.0}))
print(acc_)